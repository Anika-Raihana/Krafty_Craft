...
def add_to_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            order_item[0].save()
            messages.info(request, "This item was already in your cart. The quantity was updated!!")
            return redirect("App_Shop:home")
        else:
            order.orderitems.add(order_item[0])
            messages.info(request, "This item has been added to your cart successfully!!")
            return redirect("App_Shop:home")
    else:
...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            order_item.delete()
            messages.warning(request, "This item has been removed form your cart successfully")
            return redirect("App_Order:cart")
        else:
            messages.info(request, "This item was not found in your cart.")
            return redirect("App_Shop:home")
    else:
        messages.info(request, "Your order is not active!")
        return redirect("App_Shop:home")
